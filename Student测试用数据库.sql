...
-- 分页查询 正序排列
SELECT TOP 5 * FROM classInfo
WHERE cid NOT IN
(
	SELECT TOP(5*(1-1)) cid FROM classInfo ORDER BY cid ASC
)ORDER BY cid ASC
...
WHERE sid NOT IN
(
	SELECT TOP(5*(1-1)) sid FROM student ORDER BY sid ASC
)ORDER BY sid ASC

INSERT INTO student(sid,sname,ssex,sage,sbirthday,sphone,saddress,scid,spassword) VALUES('Admin',1,DATEDIFF(YY, '1990-01-01', GETDATE()),'1990-01-01','13888888888','湖南长沙',1,'123456');
...
WHERE tid NOT IN
(
	SELECT TOP(5*(1-1)) tid FROM teacher ORDER BY tid ASC
)ORDER BY tid ASC
...
WHERE cid NOT IN
(
	SELECT TOP(5*(1-1)) cid FROM Course ORDER BY cid ASC
)ORDER BY cid ASC
...
-- 分页查询 正序排列
SELECT TOP 5 * FROM SC_Mapping
WHERE scid NOT IN
(
	SELECT TOP(5*(1-1)) scid FROM SC_Mapping ORDER BY scid ASC
)ORDER BY scid ASC
...
INSERT INTO users VALUES('张三',GETDATE());
INSERT INTO users VALUES('李四',GETDATE());
INSERT INTO users VALUES('王五',GETDATE());
